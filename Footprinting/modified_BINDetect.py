...
# Frances Heredia
# Franco Lab
# Description: BINDetect: Detects differential binding between conditions as 
# well as bound transcription factors from footprints and motifs

# This is a modified version of TOBIAS BINDetect created by Mette Bentsen.
# @contact: mette.bentsen (at) mpi-bn.mpg.de

# Modifications:
# 1. Introduced a single dedicated writer process (file_writer_worker) consuming from one manager.Queue (write_q) 
#    and a batched serial writer function (file_writer_serial) that writes TF tmp files in batches (default batch size = 500).
# 2. Batching of motif/Tf file writes to avoid opening many files simultaneously (batch size configurable in code).
# 3. Added a helper file_writer_serial(tf_files_dict, args) (new) — writes TF files without using multiprocessing queues.
# 4. Added a worker process file_writer_worker that continuously consumes (TF_name, bed_content) tuples and writes to disk — started as writer_proc.
# 5. Removed / reduced reliance on writer_pool writer subprocesses and the previous writer_tasks mechanism; replaced with 
#    a simpler serialized/batched writing approach.
# 6. Behavioral change: writer_qs is set to None in new code and scanning calls are passed writer_qs=None (so scan_and_score
#    must either accept None or use args.log_q/other side-effects). The new code writes motif tmp files in batches outside 
#    the main parallel scanning, reducing simultaneous open files.

# USAGE: Install TOBIAS as per TOBIAS documentation, navigate to the folder within conda where the bindetect.py 
# Rename this script as bindetect.py and replace the original bindetect.py file with this modified version. 
# Then run TOBIAS BINDetect as per normal instructions.

# NOTE: This version can only use one core (i.e., --cores 1) 
################################################################################
...
def file_writer_serial(tf_files_dict, args):
	"""
	Write TF motif files directly without using a Queue.
	This is safe for batched/serialized execution.
	"""
	for TF, file in tf_files_dict.items():
		# Make sure output directory exists
		outdir = os.path.dirname(file)
		os.makedirs(outdir, exist_ok=True)

		# Write the file (this is what your original file_writer does)
		# For example, write tmp file first then rename (if needed)
		# Adjust the content logic from original file_writer
		open(file, "a").close()
#
#----------------------------------------------------------------------------------------------------------------#
def run_bindetect(args):
	...
	# ---------- Insert / replace this block in run_bindetect (where you set up writer queues) ----------

	# Create a single multiprocessing manager queue for writing
	manager = mp.Manager()
	write_q = manager.Queue()

	def file_writer_worker(q, args):
		"""
		Worker process that consumes (TF_name, bed_content) tuples from q
		and writes them to the appropriate TF tmp file. Opens/closes files
		for each write to avoid many concurrently-open file descriptors.
		"""
		while True:
			item = q.get()
			if item is None:
				# None used as global sentinel to stop worker
				break
			# item is expected to be (name, bed_content)
			name, bed_content = item
			try:
				# derive file path the same way original code did
				out_file = os.path.join(args.outdir, name, "beds", f"{name}.tmp")
				# make sure directory exists
				os.makedirs(os.path.dirname(out_file), exist_ok=True)
				# append content (bed_content is already a string with newline)
				with open(out_file, "a") as fh:
					fh.write(bed_content)
			except Exception as e:
				# Best to log to the logger queue if available, otherwise print
				try:
					# logger.queue was set to args.log_q earlier, but we are in a worker process — use stderr
					sys.stderr.write(f"Error writing file for {name}: {e}\n")
				except:
					pass
		# worker exits
		return 0
	...
	writer_proc = mp.Process(target=file_writer_worker, args=(write_q, args), daemon=False)
	writer_proc.start()

	# Create writer_qs mapping expected by scan_and_score:
	# map each motif name to the shared write_q (manager.Queue proxy)
	#writer_qs = {tf: write_q for tf in motif_names}
	writer_qs = None
	results = []
	...
	# NOTE: scan_and_score expects the dict named writer_qs (in your original call you pass writer_qs)
	# so now pass this writer_qs to it (you already supply writer_qs later in your scanning calls).
	# -----------------------------------------------------------------------------------------------
	...
	batch_size = 500  #  Number of TFs to process per batch
	logger.info(f"Writing in batches of {batch_size} motifs to avoid open-file limits")



	for start in range(0, len(motif_names), batch_size):
		end = min(start + batch_size, len(motif_names))
		TF_batch = motif_names[start:end]
		files_batch = [os.path.join(args.outdir, TF, "beds", TF + ".tmp") for TF in TF_batch]
		logger.debug(f"Processing batch {start//batch_size + 1}: {len(TF_batch)} TFs")
		file_writer_serial(dict(zip(TF_batch, files_batch)), args)
	...
	logger.debug("Stopping writer process")
	write_q.put(None)   # signal the writer process to exit
	writer_proc.join(timeout=300)
	if writer_proc.is_alive():
		writer_proc.terminate()
		writer_proc.join()
		logger.warning("Writer process had to be terminated.")
	else:
		logger.debug("Writer process finished cleanly.")
	...
	writer_proc.join(timeout=300)
	if writer_proc.is_alive():
		# if still alive after timeout, terminate and log
		writer_proc.terminate()
		writer_proc.join()
		logger.warning("Writer process had to be terminated.")
	else:
